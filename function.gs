...
function new_quiz_sv(sheetName, min, max, way){
  ...
  do {
    ...
    if(way<3){
      ...
      var count_q     = sheet.getRange("F" + r).getValue();
      var right_q     = sheet.getRange("G" + r).getValue();
      var ok          = sheet.getRange("H" + r).getValue();
    }else{
      var text_quiz   = sheet.getRange("I" + r).getValue() + "-" + 
                        sheet.getRange("B" + r).getValue() + "<br/>" + 
                        sheet.getRange("C" + r).getValue();
  ...
  // 出題回数を1増加
  sheet.getRange("F" + r).setValue(count_q + 1);
    ...
    case 3:
      ...
      for(var i = 0; i < 4; i++){
        if(i == right_sel){
          ...
          s[i] = sheet.getRange("D" + Math.floor(Math.random() * (max+1-min) + min + 1)).getValue();
        }
      }
      break;
    default:
