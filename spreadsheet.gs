...
function getPoint(userId, contents, qNum){
  ...
  if(qNum!=null){
    qNum++;
    if(contents!="DUO"){
      count_q     = sheet.getRange("F" + qNum).getValue();
      right_q     = sheet.getRange("G" + qNum).getValue();
      sheet.getRange("G" + qNum).setValue(right_q + 1);
    }
    if(userId!=null){
...
function addAns(sheetName,ans){
  ...
  sheet.getRange("C" + write_r).setValue(ans);
}
//説明追加
function addQuiz(sheetName,quiz){
  ...
  var sheet = sheetOpen.getSheetByName(sheetName);
  var write_r = sheet.getLastRow();
  sheet.getRange("D" + write_r).setValue(quiz);
  sheet.getRange("H" + write_r).setValue("1");
}
